// Shared archive-fetch logic usable from both browser and server.
import { resolvePostImageUrl, rewriteLegacyUrl } from "@/lib/legacy-urls";
import type {
  ArchiveItem,
  ArchiveHeader,
  ArchivePayload,
} from "@/serverFns/archives.functions";
...
export async function fetchArchiveViaRpc(
  client: any,
  input: ArchiveInput,
): Promise<ArchivePayload | null> {
  const page = clampPage((input as any).page);
