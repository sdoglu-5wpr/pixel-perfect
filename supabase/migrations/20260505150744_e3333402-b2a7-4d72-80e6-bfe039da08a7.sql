create or replace function public.get_archive_list(p_kind text, p_slug text default null, p_year integer default null, p_month integer default null, p_day integer default null, p_q text default null, p_page integer default 1, p_page_size integer default 10)
...
create or replace function public.get_homepage_data(p_section_slugs text[] default array['pr-news','pr-insights','marketing','social-media'], p_crisis_slug text default 'crisis-pr', p_economy_slug text default 'corporate-pr')
  ...
  left join lateral (
    ...
    from (
      select pp.id, pp.slug, pp.title, pp.excerpt, pp.published_at,
             pp.first_inline_image as content_html,
