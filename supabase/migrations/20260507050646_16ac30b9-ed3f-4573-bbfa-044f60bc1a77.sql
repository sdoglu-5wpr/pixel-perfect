CREATE OR REPLACE FUNCTION public.get_homepage_data(
  p_section_slugs text[] DEFAULT ARRAY['pr-news'::text, 'pr-insights'::text, 'marketing'::text, 'social-media'::text],
  p_crisis_slug text DEFAULT 'crisis-pr'::text,
  p_economy_slug text DEFAULT 'corporate-pr'::text
)
RETURNS jsonb
LANGUAGE sql
